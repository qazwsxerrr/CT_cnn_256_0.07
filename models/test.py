...
def plot_result(idx, f_true, f_init, f_pred, res_init, res_pred, save_path, noise_desc, lambda_reg):
    ...
    im0 = axes[0].imshow(f_true, cmap="gray", origin="lower")
    axes[0].set_title("True Image")
    plt.colorbar(im0, ax=axes[0])

    im1 = axes[1].imshow(f_init, cmap="gray", origin="lower")
    axes[1].set_title(f"Tikhonov Init\nRES={res_init:.4f}")
    plt.colorbar(im1, ax=axes[1])

    im2 = axes[2].imshow(f_pred, cmap="gray", origin="lower")
    axes[2].set_title(f"Reconstruction\nRES={res_pred:.4f}")
    plt.colorbar(im2, ax=axes[2])

    plt.tight_layout()
    plt.suptitle(f"Sample {idx} | {noise_desc} | λ={lambda_reg}", y=1.08)
...
def evaluate_shepp_logan(
    ...
    # Resolve noise settings from args or config
    mode = noise_mode if noise_mode is not None else DATA_CONFIG.get("noise_mode", "snr")
